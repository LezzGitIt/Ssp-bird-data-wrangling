...
-   All co-authors:
    ...
    -   It seems like 'Boyacá & Santander' isn't really an 'ecoregion' in the way that the others are (e.g. Bajo Magdalena refers to part of a river valley). To a lesser extent, the 'Ecoregion cafetera' seems similar given that this name comes from the predominant crop (agricultural) / cultural similarities.
...
-   Christina & Claire:
    ...
    -   Should we hold off on publishing these data sets for a while? We cannot put an embargo on the data so once accepted the data will be made public immediately.
