...
Data$Site_covs %>%
  ...
  # Spanish primary name + English descriptor, matching the ecoregion list in the site description
  mutate(Ecoregion = case_match(Ecoregion,
    "Cafetera"            ~ "Coffee region (Central Andes)",
    "Cordillera oriental" ~ "Cordillera Oriental (Eastern Andes)",
    "Piedemonte"          ~ "Piedemonte (Orinoco Basin piedmont)",
    "Bajo magdalena"      ~ "Bajo Magdalena (lower basin of the Magdalena River)",
    "Rio cesar"           ~ "Río Cesar (Cesar river valley)"
  )) %>%
  arrange(Range) %>%
