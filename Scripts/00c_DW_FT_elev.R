...
load("Rdata/Taxonomy_09.15.25.Rdata")
...
Elev_ranges <- Tax_df2[, c("Species_ayerbe", "Species_bt", "Species_eb", "Species_bl", "Match_type")] %>%
  ...
  distinct(
    ...
    Min_alt_B20, Max_alt_B20, lower_eB, upper_eB, Match_type, Year
  ) %>%
  group_by(Species_ayerbe) %>%
  summarize(
    Min_elevQJ = min(Min_elevQJ, na.rm = T), 
    Max_elevQJ = max(Max_elevQJ, na.rm = T),
    lower_eB = min(lower_eB, na.rm = T), 
    upper_eB = max(upper_eB, na.rm = T),
    Min_alt_B20 = min(Min_alt_B20, na.rm = T), 
    Max_alt_B20 = max(Max_alt_B20, na.rm = T),
    across()
  ) %>%
  arrange(Year) %>%
  slice_head() %>%
  ...
  mutate(Diff_QJ_eB = abs(elev_range_eB - elev_range_QJ)) %>% 
  ...
  rowwise() %>%
  mutate(
    Max_elev_comb = max(c_across(cols = c(Max_elevQJ, upper_eB)), na.rm = T), 
    Min_elev_comb = min(c_across(cols = c(Min_elevQJ, lower_eB)), na.rm = T), 
    # Take the largest elevational range from the 2 sources 
    Source_comb_elev = case_when(
      Max_elevQJ > upper_eB & Min_elevQJ < lower_eB ~ "QJ",
      Max_elevQJ < upper_eB & Min_elevQJ > lower_eB ~ "Freeman",
      # Source_comb_elev == 'Both' if min / max come from different sources
      Max_elevQJ >= upper_eB & Min_elevQJ >= lower_eB |
        Max_elevQJ <= upper_eB & Min_elevQJ <= lower_eB ~ "Both"
    )
  ) %>%
  mutate(Year = ifelse(Source_comb_elev == "Freeman", "2022", Year)) %>%
...
# Extras ------------------------------------------------------------------
# >Differences QJ and eBird -----------------------------------------------
# TO DO: With guide book, it is worth checking the species with extreme differences in ranges. Use Hilty to get a third range to help decide whether eBird or QJ is more trustworthy in each case.  

Elev_ranges <- read_xlsx( "Derived/Excels/Elev_ranges_diff_qj_ebird.xlsx")

Tax_join <- Tax_df2 %>% distinct(Order_gbif, Family_gbif, Species_ayerbe)

Elev_lrg_diffs <- Tax_join %>% 
  full_join(Elev_ranges) %>%
  filter(is.na(Diff_QJ_eB) | Diff_QJ_eB > 500) %>% 
  #select(Species_ayerbe, elev_range_eB, elev_range_QJ, Diff_QJ_eB) %>% 
  #select(-c(Species_bt, Species_eb, Species_bl, Match_type, contains("comb"))) %>%
  arrange(desc(Diff_QJ_eB))
Elev_lrg_diffs %>% 
  as.data.frame() %>%
  xlsx::write.xlsx("Derived/Excels/Elev_ranges_diff_qj_ebird2.xlsx", 
                   row.names = FALSE, showNA = FALSE)
  
Elev_lrg_diffs %>% tabyl(Source_comb_elev)
