...
We used Anne Chao's iNEXT 4 steps package to generate asymptotic & non-asymptotic estimates of diversity for each assemblage in the SCR project. Chao's framework relies on the concept of sample completeness, where the order q defines how heavily abundances of species are weighted. For example, at q = 0, completeness refers to 'the ratio of the observed species richness to the true richness (observed plus undetected', but can be generalized for any order q ≥ 0 extends Turing's sample coverage and quantifies the proportion of the assemblage's individuals belonging to detected species, with each individual being proportionally weighted by the (q− 1)th power of its abundance. & or 'the proportion of the total number of individuals in the entire assemblage that belongs to detected species (Chao et al., 2020)'. This allows We define an assemblage as a \[farm \* unique database \* AñoGrp\], for example 3886.GAICA Distancia.2019. Bird abundances were summed for each assemblage before calculation of diversity metrics (see aggregate data section).
...
Birds_analysis <- read_excel("Derived/Excels/Birds_analysis.xlsx")
# Temporary data wrangling DELETE
Birds_analysis <- Birds_analysis %>% rename(Species_ayerbe = Nombre_ayerbe)
...
# Remove farms on the second run through 
first_run <- FALSE
```
...
## Remove farms low SC {#sec-Remove-farms-low-SC}

After running through this analysis once, I noticed there were several farms that had very low sample coverage (SC). I remove the 9 \[farm \* unique database \* AñoGrp\] with observed SC \< .65 in order to increase the Cmax value (see @sec-Cmax).

```{r}
#| label: Rm-farms

if(first_run == FALSE){
Rm_farms <- results_no_rm %>% 
  arrange(SC_obs) %>% 
  filter(SC_obs < .65) %>% 
  # For visualization / clarity, doesn't impact results 
  slice_min(SC_obs, by = all_of(Row_id_inext)) %>% 
  distinct(across(all_of(Row_id_inext)), SC_obs) %>% 
  mutate(Uniq_db = str_replace_all(Uniq_db, "_", " "),
         Ano_grp = str_replace_all(Ano_grp, "_", "-"))

Birds_analysis <- Birds_analysis %>% anti_join(Rm_farms)
}

```

Assemblages removed due to low sample coverage:

```{r}
#| label: visualize-rm-farms

if(first_run == FALSE){
Rm_farms %>% arrange(Uniq_db, Id_gcs)
}
```

Note that while this is 8 assemblages, there is only one farm that is actually removed (Cipav 1262), b/c the other farms all other sampled in other periods.
...
date_join <- Pc_date8 %>%
  select(all_of(Row_id_inext), Fecha, Season) %>% #Season
  distinct()
Birds_analysis2 <- Birds_analysis %>% 
  left_join(date_join) %>% 
  filter(Season != "Late")
...
date_join <- Event_covs %>% 
  left_join(Site_covs) %>%
  select(all_of(Row_id_inext), Fecha, Rep_ano_grp) %>% #Season
  distinct()
Birds_analysis2 <- Birds_analysis %>% 
  left_join(date_join) %>% 
  filter(Rep_ano_grp < 5)
}
...
pivot_wider_inext <- function(row_id, level.farm = FALSE){
  if(level.farm){
    Birds_analysis2 <- Birds_analysis2 %>% filter(!is.na(Id_gcs))
  }
  Birds_analysis2 %>%
    ...
    summarize(Count = sum(Count), 
              .by = c(Identifier, Species_ayerbe, Fecha, Rep)) %>%
    # Sum across multiple surveys on separate days
    summarize(Count = sum(Count),
              .by = c(Identifier, Species_ayerbe)) %>%
...
length(unique(Birds_analysis2$Species_ayerbe))
```
...
Mult_dc_farms <- Birds_analysis2 %>% count(Uniq_db, Id_gcs) %>%
  count(Id_gcs, sort = T) %>% 
  filter(n > 1)
```
...
Even after removing farms with low SC (see @sec-Remove-farms-low-SC), we have a Cmax value of `r Cmax_num`, which is pretty low compared to Chao et al. (2020). **This means that for species richness, we can make valid comparisons across assemblages up to a `r Cmax_num`% assemblage fraction.**

Back to the code... Name the "Assemblage' column, pivot_longer so Order.q is a single column. This permits joining with i4steps\$summary\[\[2\]\] in next step
...
```{r}
#| lst-label: lst-summary-df
#| lst-cap: Dataframe that we will ultimately export. 
...
# Preliminary analysis of biases
...
Export @lst-summary-df (i.e. summary_df) for sharing with collaborators
...
```{r}
#| label: Tracking-farms

if(FALSE){
inext_farms <- bw_df_farm %>% colnames() %>% 
  str_split_i("\\.", i = 2) %>% 
  unique() %>% 
  tibble(Id_gcs = .) #%>% 
  #standardize_column_contents(col = Id_gcs)

Ecotropico_dfs <- Bird_pcs %>% pull(Id_gcs) %>% 
  unique() %>% 
  tibble(Id_gcs = .) %>% 
  mutate(Id_gcs = str_replace(Id_gcs, " ", "_"))

inext_farms %>% full_join(Ecotropico_dfs, keep = TRUE) %>% 
  rename_with(~ c("Id_inext", "Id_ecotropico")) %>% 
  mutate(Same = Id_inext == Id_ecotropico)
  #write.xlsx(file = "Derived/Id_gcs.xlsx")
}
```
