...
# NOTES:: This script can be ran for each unique data base (e.g., if giving products to data collector for their review) or with everything merged (e.g., for me to work on, to review with Nick). There are a few places where code must be changed which can be found by searching:
# "IF data collector" (followed by 'merged' or 'separate')
...
ElevOut <- spp_obs_sf_El %>%
  ...
  filter(Out != "FALSE") %>%
  group_by(Nombre_ayerbe, Out, Uniq_db) %>% # Uniq_db,
  mutate(Amount_out_m = max(Amount_out_m, na.rm = T)) %>%
  distinct(
    Nombre_ayerbe, Min_elev_comb, Max_elev_comb, Uniq_db, # Uniq_db,
    Out, Amount_out_m, Elev_range_year, Source_elev
  ) %>%
  arrange(Uniq_db, desc(Amount_out_m)) # Uniq_db,
...
# This is the key data frame to cycle through each species separately for each data base
## IF data collectors separate... Run this
SppDb <- spp_obs_sf3 %>%
  st_drop_geometry() %>%
  group_by(Nombre_ayerbe, Uniq_db) %>%
  slice_head() %>%
  select(Nombre_ayerbe, Uniq_db) %>%
  arrange(Uniq_db) %>%
  as.data.frame()
...
SppDb <- SppDb %>% filter(!Nombre_ayerbe %in% c("Leptotila verreauxi", "Sirystes sibilator", "Accipiter bicolor"))
...
length(aspp_obs_sf)
names(aspp_obs_sf) <- paste(SppDb$Nombre_ayerbe, SppDb$Uniq_db, sep = "_")
names(range) <- paste(SppDb$Nombre_ayerbe, SppDb$Uniq_db, sep = "_")
...
for (p in 1:nrow(DistIndEl)) {
  ...
  ## IF data collectors separate... Run this
   if(str_detect(DistIndEl$Uniq_db[p], "GAICA")){
   dist_plots[[p]] <- dist_plots[[p]] + annotate("text", x = Inf, y = Inf, label = str_split_1(DistIndEl$Uniq_db[p], pattern = " ")[2], size = 2, vjust=1, hjust=1) + theme(axis.title = element_blank())
   }
...
Nums2 <- Nums %>%
  mutate(Inst = c("CIPAV", rep("GAICA", 2), "UBC", "UBC_GAICA", "UniLlanos")) %>%
  group_by(Inst) %>%
  summarize(min_i = min(min_i), max_i = max(max_i)) # Inst = Institution
length(dist_plots)
for (i in 1:nrow(Nums2)) { #
pdf(file = paste0("/Users/aaronskinner/Library/CloudStorage/OneDrive-UBC/Grad_School/PhD/Analysis/Colombia-SCR-Rd3/Intermediate_products/Fuera_rango/Maps/Separate/", Nums2$Inst[i], "/DistributionsPCs_", Nums2$Inst[i], "2_", format(Sys.Date(), "%m.%d.%y"), ".pdf"), width = 8.5, height = 11, bg = "white")
...
# >Confirm missing species are within range -----------------------------------------------
# SKIP - Once visualized no need to continue to visualize
# Check guide books & other sources to understand these 2 species..
# From Avibase: 'This taxon is considered a subspecies of Sirystes [sibilator, albocinereus or subcanescens] (sensu lato) by some authors'
